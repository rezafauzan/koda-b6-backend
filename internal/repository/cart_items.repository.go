...
func (c *CartItemRepository) AddItem(item dto.CreateCartItemRequestDTO) (models.CartItem, error) {
	sql := `INSERT INTO cart_items (cart_id, product_id, size_id, variant_id, quantity, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, NOW(), NOW()) RETURNING id, cart_id, product_id, size_id, variant_id, quantity, created_at, updated_at`

	rows, err := c.db.Query(context.Background(), sql, item.CartId, item.ProductId, item.SizeId, item.VariantId, item.Quantity)
	if err != nil {
...
func (c *CartItemRepository) GetCartItemsByCartId(cartId int) ([]models.CartItem, error) {
	sql := `SELECT id, cart_id, product_id, size_id, variant_id, quantity, created_at, updated_at FROM cart_items WHERE cart_id = $1 ORDER BY created_at ASC`
...
func (c *CartItemRepository) DeleteItem(id int, cartId int) (models.CartItem, error) {
	sql := `DELETE FROM cart_items WHERE id = $1 AND cart_id = $2 RETURNING id, cart_id, product_id, size_id, variant_id, quantity, created_at, updated_at`

	rows, err := c.db.Query(context.Background(), sql, id, cartId)
	if err != nil {
		return models.CartItem{}, err
	}
...
func (c *CartItemRepository) ClearCartItem(cartId int) ([]models.CartItem, error) {
	sql := `DELETE FROM cart_items WHERE cart_id = $1 RETURNING id, cart_id, product_id, size_id, variant_id, quantity, created_at, updated_at`
