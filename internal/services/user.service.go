...
func (u UserService) CreateNewUser(newUser dto.CreateUserDTO) (dto.CreateUserDTO, error) {
	...
	_, emailExist, _ := u.userRepo.GetUserByEmail(newUser.Email)

	if emailExist {
		return dto.CreateUserDTO{}, errors.New("Failed to create new user! : Email allready used !")
	}

	_, phoneExist, _ := u.userRepo.GetUserByPhone(newUser.Phone)
